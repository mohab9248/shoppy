...
export default function UpdateProfile({setOpen, setSuccess}) {
  const {user} = useUser();
  ...
  const handleUpdate = async () => {
    ...
    try {
      const response = await fetch(
        `http://10.0.2.2:4000/userUpdate/${user._id}`,
        {
          method: 'PUT',
          headers: {
      ...
      if (response.ok) {
        // If update is successful, handle success actions
        setOpen(false);
        setSuccess(true);
        ...
        <View
          ...
          <View style={{display: 'flex', gap: 2}}>
            ...
            <TextInput
              ...
              value={phoneNumber}
              onChangeText={phoneNumber => setLastName(phoneNumber)}
              style={{
                backgroundColor: '#d7d7d7',
                height: 40,
                borderBottomWidth: 2,
